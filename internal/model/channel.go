...
// ChannelKeyMode 渠道多 key 的选择策略
type ChannelKeyMode int

const (
	ChannelKeyModeCost           ChannelKeyMode = 0 //按总成本最低
	ChannelKeyModeRoundRobin     ChannelKeyMode = 1 //按最后使用时间轮询
	ChannelKeyModeWeightedRandom ChannelKeyMode = 2 //按权重加权随机
)
...
type ChannelKey struct {
	...
	Weight           int     `json:"weight" gorm:"default:1"` // 加权随机模式下的权重
...
// GetChannelKeys 按 KeyMode 策略返回排好序的可用 key 列表（首选在前）
...
// key429CooldownSeconds 触发 429 后该 key 的冷却时间（秒），冷却期内不会被选中
const key429CooldownSeconds int64 = 5 * 60
...
// weightedRandomSort 按加权随机分数降序排序（weight 越大被选中的概率越高）
