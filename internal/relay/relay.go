...
func (r *relayRun) run() {
	...
	for r.iter.Next() {
		...
		case <-ctx.Done():
			...
			go r.metrics.Save(context.Background(), false, context.Canceled, r.iter.Attempts())
			return
		default:
		}
		...
		if err == nil {
			go r.metrics.Save(context.Background(), true, nil, r.iter.Attempts())
			return
		}
		if written {
			go r.metrics.Save(context.Background(), false, err, r.iter.Attempts())
			return
		}
		lastErr = err
	}
	...
	go r.metrics.Save(context.Background(), false, lastErr, r.iter.Attempts())
...
func (ra *relayAttempt) forward() (int, error) {
	...
	// 首字超时:覆盖「等待上游响应头 + 首个事件」阶段(此前计时器在 writeStream 内才启动,
	// 上游在返回响应头之前挂住时不会触发,也无法故障转移)
	var streamFirstEventTimeout, nonStreamTimeout time.Duration
